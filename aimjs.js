...
function tridraw(inpng1, inpng2, outpng, incoords1, incoords2, outcoords, alfa, subpixel) {
  ...
  if (al > bl) {
    ...
    [bdx, bdy, adx, ady] = [adx, ady, bdx, bdy];
    [incoords1[0], incoords1[1], incoords1[2]] = [
      incoords1[1],
      incoords1[0],
      incoords1[2]
    ];
    [inbdx1, inbdy1, inadx1, inady1] = [inadx1, inady1, inbdx1, inbdy1];
    [incoords2[0], incoords2[1], incoords2[2]] = [
      incoords2[1],
      incoords2[0],
      incoords2[2]
    ];
    [inbdx2, inbdy2, inadx2, inady2] = [inadx2, inady2, inbdx2, inbdy2];
  }
  ...
  for (var i = 0; i < itarg; i++) {
    ...
    for (var j = 0; j < Math.floor(sl * subpixel); j++) {
      ...
      if (
        ...
        idx2 =
          (inpng1.width * Math.floor(inps1[1]) + Math.floor(inps1[0])) * 4;
        ins1 = [
          inpng1.data[idx2],
          inpng1.data[idx2 + 1],
          inpng1.data[idx2 + 2],
      ...
      if (
        ...
        idx3 =
          (inpng2.width * Math.floor(inps2[1]) + Math.floor(inps2[0])) * 4;
        ins2 = [
          inpng2.data[idx3],
          inpng2.data[idx3 + 1],
          inpng2.data[idx3 + 2],
...
function getfeaturepoints(png, n, m) {
  ...
  for (var j = 0; j <= m; j++) {
    ...
    for (var i = 0; i <= n; i++) {
      ...
      if (j > 0 && j < m && i > 0 && i < n) {
        fd = png.height + png.width; // Initialize feature point-cell center distance with a maximum
        ...
        for (var y = 0; y < ch; y++) {
          for (var x = 0; x < cw; x++) {
            idx =
              (png.width *
                Math.floor(y + j * ch - ch / 2) +
                Math.floor(x + i * cw - cw / 2)) *
              4;

            // Calculate contrast (tc) as the sum of absolute RGBA difference between this pixel and its neighbors
            tc = 0;
            tc +=
              Math.abs(png.data[idx] - png.data[idx + 4]) +
              Math.abs(png.data[idx + 1] - png.data[idx + 5]) +
              Math.abs(png.data[idx + 2] - png.data[idx + 6]) +
              Math.abs(png.data[idx + 3] - png.data[idx + 7]);
            tc +=
              Math.abs(png.data[idx] - png.data[idx - 4]) +
              Math.abs(png.data[idx + 1] - png.data[idx - 3]) +
              Math.abs(png.data[idx + 2] - png.data[idx - 2]) +
              Math.abs(png.data[idx + 3] - png.data[idx - 1]);
            tc +=
              Math.abs(png.data[idx] - png.data[idx + png.width]) +
              Math.abs(png.data[idx + 1] - png.data[idx + png.width + 1]) +
              Math.abs(png.data[idx + 2] - png.data[idx + png.width + 2]) +
              Math.abs(png.data[idx + 3] - png.data[idx + png.width + 3]);
            tc +=
              Math.abs(png.data[idx] - png.data[idx - png.width]) +
              Math.abs(png.data[idx + 1] - png.data[idx - png.width + 1]) +
              Math.abs(png.data[idx + 2] - png.data[idx - png.width + 2]) +
            ...
            // If this contrast is the same as the latest but closer, or if this contrast is bigger, update the feature point
            if (
...
function drawfeaturepoints(png, fpts, col) {
  for (var j = 0; j < fpts.length; j++) {
    for (var i = 0; i < fpts[j].length; i++) {
      var idx =
        (png.width * Math.floor(fpts[j][i][1]) +
          Math.floor(fpts[j][i][0])) *
        4;
      png.data[idx] = col[0];
...
// Get an interpolated frame from 2 source images mixed at alfa and draw it to outpng
  ...
  for (var j = 0; j < fpts1.length - 1; j++) {
    for (var i = 0; i < fpts1[j].length - 1; i++) {
      ...
      tridraw(
        ...
        ], // out triangle 2 interpolated at alfa
        alfa,
        subpixel
      );
    }
...
function anim(
  ...
  for (var i = 1; i < keyframefilenames.length; i++) {
    outpng = newpng2(iw, ih); // Create new output png
    ...
    for (var j = 0; j < framerate; j++) {
      ...
      console.log(
        filename + " saved. " + (Date.now() - starttime) + " ms"
      );
    }
  }
}
